...
deployments built on Maverick. The bar is evidence, not enthusiasm — every
...
- **It actually runs on Maverick** — the kernel (`maverick start` /
...
- **It respects the [trademark policy](../TRADEMARK.md)** — "built with
  Maverick" is fine (nominative use); naming your product "Maverick-anything"
  or implying it is the official Maverick is not.
...
| **Maverick surface** | The parts that are load-bearing: kernel, channels, MCP, gRPC, specific tools/skills. |
| **Evidence** | Link to the replayable trace or run export. |
| **Submitted by** | GitHub handle of the submitter. |

| Project | What it does | Maverick surface | Evidence | Submitted by |
|---|---|---|---|---|
