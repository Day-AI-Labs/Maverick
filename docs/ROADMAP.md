...
| **Safety** | gVisor tool sandbox · eBPF syscall monitor · memory-safe parsers · supply-chain pinning · sigstore keyless signing · out-of-process model proxy · public safety bulletin RSS · federated shield model updates · cross-run anomaly detection · tamper-evident screenshots · right-to-rectification · crash-only logging · annual safety report |
| **Ecosystem** | ACD spec v1.0 · Threads + RCS channels · Anki integration · web archive tool · GitHub repo search · Redis world-model · plugin telemetry opt-in · marketplace v2 (federated indexes) · IDE protocol unification (one MCP server, multiple editors) · generic OAuth helper · DuckDB world-model · Cloudflare Workers + Modal sandboxes |
