...
|---|---|
| **Capabilities** | Audio understanding (non-speech CLAP) · DOM accessibility-tree extractor (5-10x token cut) · plan-execute-reflect loop topology · cross-language LSP bridge · vector-store as first-class memory · speculative parallel tool calls · speech-to-action live-mic · GUI element memory · image gen + edit tools · ASR meeting listener · auto-skill distillation v2 |
