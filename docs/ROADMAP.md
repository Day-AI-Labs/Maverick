...
| **UX** | Native macOS/Windows/Linux GUI apps · browser extension · voice persona presets · multi-language voice · wizard branching paths · run gallery · replay export to MP4 · collaborative supervision (multi-user dashboard) · VS Code + JetBrains live-run extensions · TUI mouse mode · run-as-tutorial export · accessibility audit pass · i18n community portal |
| **Distribution** | Windows MSI · marketplace moderation tooling · sponsorship tiers · conference physical booth · swag store · ambassadors program · long-form handbook · Skill of the Year award · 2.0 RFC · mobile companion app v1 (read-only) · self-hosted relay reference · localized docs phase 4 (ko/ru/it/hi) · video season 3 · skill search engine (HF) · annual community survey · foundation exploration |
...
| **Safety** | gVisor tool sandbox · eBPF syscall monitor · memory-safe parsers · sigstore keyless signing · out-of-process model proxy · public safety bulletin RSS · federated shield model updates · cross-run anomaly detection · tamper-evident screenshots · annual safety report |
| **Ecosystem** | ACD spec v1.0 · Threads + RCS channels · Anki integration · Redis world-model · marketplace v2 (federated indexes) · IDE protocol unification (one MCP server, multiple editors) · DuckDB world-model · Cloudflare Workers + Modal sandboxes |
