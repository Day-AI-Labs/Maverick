...
| **Performance** | — (cleared: **compaction v6 hybrid shipped** (`compaction/hybrid.py`, ledger-learned strategy picker, fail-open) and the **sandbox pool shipped** (`sandbox/pool.py` cross-run pooling under a strict scrub contract + Firecracker e2b warm mode — engines that can't prove a clean handoff always build fresh)) |
