...
| **Performance** | — (cleared: **compaction v6 hybrid landed (experimental)** (`compaction/hybrid.py`, ledger-learned strategy picker, fail-open) — the picker is implemented + unit-tested and opt-in via `[compaction] hybrid`, but is **not yet wired into the live agent compaction path** (still the rule ladder); and the **sandbox pool shipped** (`sandbox/pool.py` cross-run pooling under a strict scrub contract + Firecracker e2b warm mode — engines that can't prove a clean handoff always build fresh)) |
