...
| **Safety** | HIPAA mode profile · shield calibration dashboard · adversarial eval harness · coordinated-disclosure log · per-agent identity + signing · image-content classifier · voice safety pass · privacy budget per user · red-team CI |
...
| **Safety** | Refusal calibration · gVisor tool sandbox · eBPF syscall monitor · memory-safe parsers · supply-chain pinning · sigstore keyless signing · out-of-process model proxy · rate-limit shield calls per goal · public safety bulletin RSS · federated shield model updates · model card per LLM · behavioral diff on upgrades · cross-run anomaly detection · tamper-evident screenshots · right-to-rectification · crash-only logging · annual safety report |
