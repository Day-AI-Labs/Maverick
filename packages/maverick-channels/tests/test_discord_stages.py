...
def _seam(speaker=False):
    ...
    return state, StageSeam(
        ...
        speak=speak, send_text=send_text,
    )


def _session(seam, *, wake=None, handler=None):
...
def test_utterance_assembled_across_segments():
    ...
    s = _session(seam)
    out = asyncio.run(_drive(s, [("u1", b"deploy the", False),
                                 ("u1", b"staging site", True)]))
