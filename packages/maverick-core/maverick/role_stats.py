...
def record_credit(credit_by_name: dict[str, float], name_to_role: dict[str, str],
    ...
    for name, c in credit_by_name.items():
        ...
        if role:
            record(role, c, path=path)


def top_roles(k: int = 5, *, min_runs: int = 2, path: Path | None = None) -> list[tuple[str, float]]:
    """Roles ranked by average credit (only those with >= ``min_runs`` samples)."""
    stats = _load(_resolve(path))
    ranked = [
        (role, st.avg_credit) for role, st in stats.items() if st.runs >= min_runs
    ]
    ranked.sort(key=lambda x: -x[1])
    return ranked[: max(1, k)]


def guidance(path: Path | None = None) -> str | None:
