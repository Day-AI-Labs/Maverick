...
Only connectors whose tool speaks the ``op``/``path``/``confirm`` schema (the
``make_rest_tool`` family) are wrapped; a bespoke connector with a different
shape is left alone rather than silently half-governed.
"""
...
def _is_rest_op_tool(tool: Tool) -> bool:
    """Whether ``tool`` speaks the op/path/confirm REST schema we can govern."""
    props = (tool.input_schema or {}).get("properties", {})
    return isinstance(props, dict) and "op" in props and "path" in props
...
def apply_governed_connectors(reg) -> list[str]:
    ...
    for name in names:
        try:
            ...
            continue
        if not _is_rest_op_tool(tool):
            log.info("governed_connectors: %s is not an op/path tool; left ungoverned", name)
            continue
