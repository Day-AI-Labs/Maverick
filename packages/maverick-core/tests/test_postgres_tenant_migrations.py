...
def test_pending_skips_already_applied():
    ...
    # N-of-M dual control (v21), cluster-wide halt (v22).
    pending = pg.pending_migrations(1)
    assert [v for v, _ in pending] == [10, 11, 13, 14, 15, 16, 17, 18, 19, 21, 22]
    # A DB at v10 still needs the remaining upgrades.
    assert [v for v, _ in pg.pending_migrations(10)] == [11, 13, 14, 15, 16, 17, 18, 19, 21, 22]
    # A DB at v11 still needs the rest of the ladder.
    assert [v for v, _ in pg.pending_migrations(11)] == [13, 14, 15, 16, 17, 18, 19, 21, 22]
    # A DB at v13 still needs goal-domain + projects + artifacts + share/etc.
    assert [v for v, _ in pg.pending_migrations(13)] == [14, 15, 16, 17, 18, 19, 21, 22]
    # A DB at v14 still needs projects + artifacts + share/signoff/origin.
    assert [v for v, _ in pg.pending_migrations(14)] == [15, 16, 17, 18, 19, 21, 22]
    # A DB at v15 still needs artifacts + share/signoff/origin + fact_history.
    assert [v for v, _ in pg.pending_migrations(15)] == [16, 17, 18, 19, 21, 22]
    # A DB at v16 still needs the share/signoff/origin + fact_history migrations.
    assert [v for v, _ in pg.pending_migrations(16)] == [17, 18, 19, 21, 22]
    # A DB at v17 still needs fact_history + rate_events.
    assert [v for v, _ in pg.pending_migrations(17)] == [18, 19, 21, 22]
    # A DB at v18 still needs the rate_events migration.
    assert [v for v, _ in pg.pending_migrations(18)] == [19, 21, 22]
    # A DB at v19 still needs dual-control + halt.
    assert [v for v, _ in pg.pending_migrations(19)] == [21, 22]
    # A DB at v21 still needs the cluster-wide halt migration.
    assert [v for v, _ in pg.pending_migrations(21)] == [22]
...
def test_schema_version_is_latest_migration():
    assert pg._PG_SCHEMA_VERSION == 22
